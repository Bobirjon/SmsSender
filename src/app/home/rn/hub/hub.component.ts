...
export class HubComponent implements OnInit {
  ...
  putLevel() {
    ...
    if (
      this.hubForm.value.region == 'г.Ташкент' ||
      this.hubForm.value.region == 'Ташкент.обл'
    ) {
    ...
    if (this.route.snapshot.params.id == null) {
      ...
      this.authService
        ...
        .subscribe((result: any) => {
          const selectedOptionReg = this.region.find(
            (region) => region.value === result['region']
          );
          ...
          if (result['lowBatteryTime'] == null) {
            ...
            lowBatteryTime = formatDate(
              result['lowBatteryTime'],
              'yyyy-MM-ddTHH:mm',
              'en'
            );
          }
          if (result['dg_start_time'] == null) {
            ...
            dg_start_time = formatDate(
              result['dg_start_time'],
              'yyyy-MM-ddTHH:mm',
              'en'
            );
          }
          if (result['sector_block_time'] == null) {
            ...
            block_time = formatDate(
              result['sector_block_time'],
              'yyyy-MM-ddTHH:mm',
              'en'
            );
          }
          if (result['power_off_time'] == null) {
            ...
            power_off_time = formatDate(
              result['power_off_time'],
              'yyyy-MM-ddTHH:mm',
              'en'
            );
          }
  ...
  tableSendBody() {
    ...
    this.tableBody = {
      ...
      reason:
        ...
        ' ' +
        this.word +
        this.hubForm.value.hubSite +
        ' ' +
        this.hubForm.value.generator,
      effect: 'С влиянием',
    ...
    if (this.hubForm.value.effectedSites !== '') {
      if (Array.isArray(this.hubForm.value.effectedSites) == false) {
        let splited: string[] = [];
        splited = this.hubForm.value.effectedSites.split(/\n|,/);
        this.tableBody.effected_sites = splited.filter(
  ...
  smsSendBody(id?: number) {
    ...
    if (
      this.hubForm.value.hubSite == '' ||
      this.hubForm.value.hubSite == undefined
    ) {
      this.word = ' ';
    } else {
    ...
    if (this.requestType == 'Проблема') {
      if (
        this.hubForm.value.AddOrCor == null ||
        this.hubForm.value.AddOrCor == undefined
      ) {
        this.SmsTextBody =
          this.hubForm.value.level.replace('P', 'П') +
          ' ' +
          this.requestType +
          ' на узловом сайте' +
          '\n' +
          ...
          '\n' +
          'Начало: ' +
          this.hubForm.value.startTime.replace('T', ' ') +
          '\n' +
          'Информирован: ' +
          this.hubForm.value.informed +
          '\n' +
          'Отправил: ' +
      ...
      if (
        ...
        this.SmsTextBody =
          ...
          '\n' +
          'Конец: ' +
          this.hubForm.value.endTime.replace('T', ' ') +
          '\n' +
          'Информирован: ' +
          this.hubForm.value.informed +
          '\n' +
    ...
    if (this.hubForm.value.periodicity == '') {
      smsType = this.storageService.SmsType(
        this.requestType,
        this.hubForm.value.AddOrCor,
        false
      );
  ...
  regionSelect(event: any) {
    const selectedOption = this.region.find(
