...
export class BscComponent implements OnInit {
  ...
  optionsEffect: string[] = ['Нет эффект на сервис ', 'Недоступен 3G'];
  ...
  constructor(
    ...
    this.filteredOptionsProblem =
      this.bscForm.controls.problem.valueChanges.pipe(
        startWith(''),
  ...
  setDefault() {
    if (
      this.bscForm.value.level == 'P1' ||
      this.bscForm.value.level == 'P2' ||
      this.bscForm.value.level == 'P3' ||
      this.bscForm.value.level == 'P4' ||
      this.bscForm.value.level == 'P5'
    ) {
  ...
  smsSendBody(id?: number) {
    if (this.requestType == 'Проблема') {
      if (
        this.bscForm.value.AddOrCor == null ||
        this.bscForm.value.AddOrCor == undefined
      ) {
        this.SmsTextBody =
          this.bscForm.value.level.replace('P', 'П') +
          ' ' +
          this.requestType +
          ':\n' +
          ...
          '\n' +
          'Причина: ' +
          this.bscForm.value.reason +
          '\n' +
          'Эффект: ' +
          this.bscForm.value.effect +
          '\n' +
          'Начало: ' +
          this.bscForm.value.startTime.replace('T', ' ') +
          '\n' +
          'Оповещен: ' +
          this.bscForm.value.informed +
          '\n' +
          'Отправил: ' +
      ...
      if (
        ...
        this.SmsTextBody =
          ...
          '\n' +
          'Описание: ' +
          this.bscForm.value.desc +
          '\n' +
          'Начало: ' +
          this.bscForm.value.startTime.replace('T', ' ') +
          '\n' +
          'Конец: ' +
          this.bscForm.value.endTime.replace('T', ' ') +
          '\n' +
          'Оповещен: ' +
          this.bscForm.value.informed +
          '\n' +
    ...
    if (
      this.bscForm.value.region != undefined ||
      this.bscForm.value.region != null
    ) {
      ...
      if (result == true) {
        if (this.newForm == false) {
          ...
          this.authService
            ...
            .subscribe(
              (result) => {
                this.snackBar.open('Обновлено в таблице', '', {
                  duration: 10000,
                });
          ...
          this.authService.postData(this.tableBody).subscribe(
            ...
            (error) => {
              console.log(error);
              this.snackBar.open('Ошибка при добавлении в таблицу ', '', {
                duration: 10000,
              });
            }
          );
