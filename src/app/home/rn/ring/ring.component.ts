...
export class RingComponent implements OnInit {
  ...
  constructor(
    ...
    this.filteredOptionsReason =
      this.ringForm.controls.reason.valueChanges.pipe(
        startWith(''),
  ...
  setDefault() {
    if (
      this.ringForm.value.level == 'P1' ||
      this.ringForm.value.level == 'P2' ||
      this.ringForm.value.level == 'P3' ||
      this.ringForm.value.level == 'P4' ||
      this.ringForm.value.level == 'P5'
    ) {
  ...
  smsSendBody(id?: number) {
    let powerOffTime =
      this.ringForm.value.powerOffTime == ''
        ? 'НД'
        : this.ringForm.value.powerOffTime.replace('T', ' ');
    ...
    this.SmsTextBody =
      ...
      `${
        this.ringForm.value.AddOrCor != null ||
        this.ringForm.value.AddOrCor != undefined
          ? '(' + this.ringForm.value.AddOrCor + ')\n'
          : ''
      }` +
      ...
      `${
        this.requestType !== 'Проблема'
          ? 'Описание: ' + this.ringForm.value.desc + '\n'
          : ''
      }` +
      ...
      `${
        this.requestType !== 'Проблема'
          ? 'Конец: ' + this.ringForm.value.endTime.replace('T', ' ') + '\n'
          : ''
      }` +
