...
export class CnComponent implements OnInit {
  ...
  constructor(
    ...
    this.filteredOptionsProblem =
      this.cnForm.controls.problem.valueChanges.pipe(
        startWith(''),
  ...
  setDefault() {
    if (
      this.cnForm.value.level == 'P1' ||
      this.cnForm.value.level == 'P2' ||
      this.cnForm.value.level == 'P3' ||
      this.cnForm.value.level == 'P4' ||
      this.cnForm.value.level == 'P5'
    ) {
  ...
  smsSendBody(id?: number) {
    this.SmsTextBody =
      ...
      `${
        this.cnForm.value.AddOrCor != null ||
        this.cnForm.value.AddOrCor != undefined
    ...
    let smsType =
      this.cnForm.value.periodicity == ''
        ? this.storageService.SmsType(
            this.requestType,
            this.cnForm.value.AddOrCor,
            false
          )
    ...
    if (
      ...
      this.cnForm.value.category == 'Core-NetAct'
    ) {
