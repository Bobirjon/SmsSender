...
export class StorageService {
  ...
  responsible_report: string[] = ['Другие ЗО', 'Эксплуатация', 'Выясняется'];

  effect_option: string[] = ['С влиянием', 'Без влияния'];

  periodicity: string[] = ['', 'периодически', 'периодически и частично'];
  ...
  login(name: string) {
    ...
    localStorage.setItem(
      'auth',
      JSON.stringify({ isAuthenticated: true, userName: this.userNameTest })
    );
  }
  ...
  public createToTable(data: any) {
    return this.authService.postData(data).subscribe(
      (result) => {
  ...
  public createToTableForSMS(data: any) {
    this.authService.postData(data).subscribe(
      (result) => {
        console.log(result);
        this.snackBar.open('Добавлен в таблицу', '', { duration: 10000 });
      },
  ...
  public updateData(id: any, data: any) {
    this.authService.updateSms(id, data).subscribe(
      (result) => {
        console.log(result);
  ...
  public sendSms(data: any) {
    this.authService.sendSms(data).subscribe(
      (res) => {
        console.log(res);
      ...
      (error) => {
        console.log(error);
        this.snackBar.open('Ошибка при отправлении сообщения', '', {
          duration: 10000,
        });
      }
    );
