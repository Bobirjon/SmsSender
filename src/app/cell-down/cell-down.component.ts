...
export class CellDownComponent implements OnInit, OnDestroy {
  ...
  applyFilter(message?: any) {
    this.cellDownTable.filterPredicate = (data: any, filter: any) => {
      return (
        (this.cellDownFilterForm.value.selectedRegion == 0 ||
          this.cellDownFilterForm.value.selectedRegion.some((reigon: any) =>
            data.region.includes(reigon.value)
          )) &&
        (!this.cellDownFilterForm.value.site ||
          data.site
            .toLowerCase()
            .includes(this.cellDownFilterForm.value.site.toLowerCase())) &&
        (!this.cellDownFilterForm.value.alarmtype ||
          data.alarmtype
            .toLowerCase()
            .includes(this.cellDownFilterForm.value.alarmtype.toLowerCase())) &&
        (this.cellDownFilterForm.value.sitesbehind === 'all' ||
          (this.cellDownFilterForm.value.sitesbehind === 'empty' &&
            (!data.sitesbehind || data.sitesbehind.trim() === '')) ||
          (this.cellDownFilterForm.value.sitesbehind === 'withValue' &&
            data.sitesbehind &&
            data.sitesbehind.trim() !== '')) &&
        (this.cellDownFilterForm.value.comment === 'all' ||
          (this.cellDownFilterForm.value.comment === 'empty' &&
            (!data.comment || data.comment.trim() === '')) ||
          (this.cellDownFilterForm.value.comment === 'withValue' &&
            data.comment &&
            data.comment.trim() !== '')) &&
        (this.cellDownFilterForm.value.power === 'all' ||
          (this.cellDownFilterForm.value.power === 'empty' &&
            (!data.power || data.power.trim() === '')) ||
          (this.cellDownFilterForm.value.power === 'withValue' &&
            data.power &&
            data.power.trim() !== '')) &&
        (this.cellDownFilterForm.value.dg === 'all' ||
          (this.cellDownFilterForm.value.dg === 'empty' &&
            (!data.dg || data.dg.trim() === '')) ||
          (this.cellDownFilterForm.value.dg === 'withValue' &&
            data.dg &&
            data.dg.trim() !== '')) &&
        (this.cellDownFilterForm.value.battery === 'all' ||
          (this.cellDownFilterForm.value.battery === 'empty' &&
            (!data.battery || data.battery.trim() === '')) ||
          (this.cellDownFilterForm.value.battery === 'withValue' &&
            data.battery &&
            data.battery.trim() !== ''))
      );
    };
  ...
  ngOnInit() {
    this.socketSubscription = this.websocket
      .getDataStream()
